...
def export_data():
    ...
    table=pandas.DataFrame(newlist,columns=['id','Name','Email','Mobile','Address','Gender','DOB','Added Date','Added Time'])
...
def toplevel_data(title,button_text,command):
    ...
    addressLabel = Label(screen, text='Adress', font=('times new roman', 20, 'bold'))
...
def update_data():
    ...
    con.commit()
    messagebox.showinfo('Success',f'Id{idEntry.get()} is modifies successfully',parent=screen)
    screen.destroy()
    show_student()
...
def connect_database():
    def connect():
        ...
        showdatestudentButton.config(state=NORMAL)
    ...
    hostEntry = Entry(connectWindow,font=('roamn',15,'bold'),bd=2)
    hostEntry.grid(row=0,column=1,padx=40,pady=20)

    usernameLabel = Label(connectWindow, text='User Name', font=('arial', 20, 'bold'))
    usernameLabel.grid(row=1, column=0, padx=20)

    usernameEntry = Entry(connectWindow, font=('roamn', 15, 'bold'), bd=2)
    usernameEntry.grid(row=1, column=1, padx=40, pady=20)

    passwordLabel = Label(connectWindow, text='Password', font=('arial', 20, 'bold'))
    passwordLabel.grid(row=2, column=0, padx=20)

    passwordEntry = Entry(connectWindow, font=('roamn', 15, 'bold'), bd=2)
...
sliderLabel = Label(root,text=s,font=('arial',28,'bold','italic'),width=30)
...
showdatestudentButton = ttk.Button(leftFrame,text='Show student',width=25,state=DISABLED,command=show_student)
showdatestudentButton.grid(row=5,column=0,pady=20)
...
style.configure('Treeview',rowhieght=40,font=('arial',12,'bold'),background='white',fieldbackground='white')
